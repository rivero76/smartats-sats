...
export const LogCleanupManager = () => {
  ...
  const { data: logStats } = useQuery({
    ...
    queryFn: async () => {
      ...
      const scriptStats = data.reduce((acc, entry) => {
        acc[entry.script_name] = (acc[entry.script_name] || 0) + 1;
        return acc;
      }, {} as Record<string, number>);

      const oldestEntry = data.length > 0 ? new Date(Math.min(...data.map(e => new Date(e.created_at).getTime()))) : null;
    ...
    <div className="space-y-6">
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {logStats?.oldestEntry ? 
                Math.ceil((Date.now() - logStats.oldestEntry.getTime()) / (1000 * 60 * 60 * 24)) : 0
              }
            </div>
            ...
            {Object.entries(logStats?.scriptStats || {}).map(([script, count]) => (
              <div key={script} className="flex items-center justify-between p-2 border rounded">
                ...
                <div className="flex items-center gap-2">
                  <span className="text-sm text-muted-foreground">{count.toLocaleString()} entries</span>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Delete old log entries to free up storage space
          </CardDescription>
        </CardHeader>
            ...
            <AlertDialog>
              ...
              <AlertDialogContent>
                <AlertDialogHeader>
                  ...
                  <AlertDialogDescription className="space-y-2">
                    <p>
                      ...
                      {selectedScript !== 'all' ? ` for script "${selectedScript}"` : ' for all scripts'}.
                    </p>
                ...
                <AlertDialogFooter>
                  ...
                  <AlertDialogAction
                    onClick={() => deleteLogsMutation.mutate({ script: selectedScript, days: retentionDays })}
