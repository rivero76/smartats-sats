...
> Generated: 2026-03-27 | Sources: docs/, plans/, src/pages/, src/components/, README.md
...
### Medium-term (P21 — Database Enterprise Foundation)

| Feature | Description |
|---|---|
| **Universal Audit Trigger (S1)** | DB-level audit trigger capturing all mutations across all tables automatically |
| **LLM Call Logs Table (S1)** | Persistent logs of every LLM call with model, tokens, cost, and duration |
| **RBAC Tables (S2)** | Role-based access control tables enabling fine-grained permissions |
| **API Key Management (S2)** | Scoped API keys for programmatic access to SmartATS data |
| **Multi-tenancy Foundation (S3)** | `sats_tenants` table enabling organization-level accounts |
| **Plans & Subscriptions (S3)** | Formal subscription plan tracking and enforcement at database level |
| **Tenant ID on all tables (S3)** | `tenant_id` column propagated to all core tables for true data isolation |

### Future / Long-term (Q2+ 2026)

| Feature | Plan | Description |
|---|---|---|
| **Global v1 Readiness** | P8 | i18n/l10n, regional compliance (GDPR, CCPA), multi-currency |
| **AI Runtime Governance** | P9 | LLM ops analytics, model performance monitoring, runtime A/B testing |
| **JD ETL Expansion** | P11 | CSV, XLS, email parsing, RSS feed, browser extension for job clipping |
| **Multi-language Support** | P12 | Resume creation and analysis in multiple languages |
| **Enterprise Scale** | P12 | Multi-cloud deployment, horizontal scaling, enterprise SLA |
| **pgvector + RAG (S5)** | P21 | Vector embeddings for semantic resume/job matching; AI agent infrastructure |
| **Modern API Patterns (S6)** | P21 | Idempotency keys, outbox pattern, rate limiting for public API |
...
- Data Export
- API Key Generation *(scoped to P17 S2)*
...
## Implied Pricing Architecture

No formal pricing tiers are currently published, but the architecture implies:

| Tier | Indicators |
|---|---|
| **Free** | Basic resume scoring, limited analyses per month |
| **Pro** | Unlimited scoring, enrichment, LinkedIn import, upskilling roadmaps |
| **BYOK / Zero-COGS** | User brings their own API key — no LLM cost to platform (P17) |
| **Enterprise** | Multi-tenancy (P21), RBAC, API access, SSO, audit logs, SLA |

---

*Sources: `docs/decisions/product-roadmap.md`, `plans/`, `src/pages/`, `src/components/`, `docs/changelog/CHANGELOG.md`, `README.md`*
